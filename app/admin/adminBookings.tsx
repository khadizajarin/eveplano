...
> = {
  ...
  confirmed: {
    label: "Confirmed",
    color: "#1a6e3c",
    bg: "rgba(26,110,60,0.12)",
    icon: "✅",
  },
  completed: {
    label: "Completed",
    color: "#041e4b",
    bg: "rgba(4,30,75,0.10)",
    icon: "🎉",
  },
  cancelled: {
    label: "Cancelled",
...
const ALL_STATUSES: ProgressType[] = [
  "pending",
  "confirmed",
  "completed",
  "cancelled",
...
const BookingCard: React.FC<{
  ...
  const handleStatusChange = async (newStatus: ProgressType) => {
    ...
    try {
      ...
      ToastAndroid.show(
        `Status updated to ${PROGRESS_CONFIG[newStatus].label}`,
        ToastAndroid.SHORT,
      );
    } catch (err) {
      console.error(err);
        ...
        >
          <View style={styles.cardTop}>
            ...
            <View style={{ alignItems: "flex-end", gap: 8 }}>
              <View
                style={[styles.progressBadge, { backgroundColor: prog.bg }]}
              >
            ...
            {ALL_STATUSES.map((status) => {
              ...
              return (
                ...
                >
                  ...
                  {isCurrent && (
                    <Text style={styles.currentTick}>✓ Current</Text>
                  )}
                </TouchableOpacity>
              );
            })}
      ...
      {isAdmin && (
        <View style={styles.adminBadge}>
          ...
          <Text style={styles.adminBadgeText}>
            Admin View — All bookings visible
          </Text>
        </View>
      )}
        ...
        <View style={styles.statsRow}>
          {ALL_STATUSES.map((status) => {
            ...
            return (
              ...
              >
                <Text style={[styles.statCount, { color: cfg.color }]}>
                  {count}
                </Text>
      ...
      {filtered.length === 0 ? (
        <View style={styles.emptyBlock}>
          <Text style={styles.emptyIcon}>
            {activeFilter === "all"
              ? "📭"
              : (PROGRESS_CONFIG[activeFilter as ProgressType]?.icon ?? "📭")}
          </Text>
...
const styles = StyleSheet.create({
  scroll: { flex: 1, backgroundColor: CREAM, paddingHorizontal: 20 },
  ...
  pickerOverlay: {
    flex: 1,
    backgroundColor: "rgba(4,30,75,0.55)",
